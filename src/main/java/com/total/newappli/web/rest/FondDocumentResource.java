...
import com.total.newappli.web.rest.util.PaginationUtil;
...
/**
 * REST controller for managing FondDocument.
 */
@RestController
@RequestMapping("/api")
public class FondDocumentResource {
    ...
    /**
     * POST  /fond-documents : Create a new fondDocument.
     *
     * @param fondDocumentDTO the fondDocumentDTO to create
     * @return the ResponseEntity with status 201 (Created) and with body the new fondDocumentDTO, or with status 400 (Bad Request) if the fondDocument has already an ID
    ...
    /**
     * PUT  /fond-documents : Updates an existing fondDocument.
     *
     * @param fondDocumentDTO the fondDocumentDTO to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated fondDocumentDTO,
     * or with status 400 (Bad Request) if the fondDocumentDTO is not valid,
     * or with status 500 (Internal Server Error) if the fondDocumentDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<FondDocumentDTO> updateFondDocument(@Valid @RequestBody FondDocumentDTO fondDocumentDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, fondDocumentDTO.getId().toString()))
            .body(result);
    }

    /**
     * GET  /fond-documents : get all the fondDocuments.
     *
     * @param pageable the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of fondDocuments in body
     */
    ...
    public ResponseEntity<List<FondDocumentDTO>> getAllFondDocuments(Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/fond-documents");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }

    /**
     * GET  /fond-documents/:id : get the "id" fondDocument.
     *
     * @param id the id of the fondDocumentDTO to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the fondDocumentDTO, or with status 404 (Not Found)
     */
    ...
    /**
     * DELETE  /fond-documents/:id : delete the "id" fondDocument.
     *
